...
class FileAdapterBuilder(DatasetBuilder):
  ...
  def _tfrecords_reader(self):
    return tfrecords_reader.Reader(self._example_specs)
  ...
  def _download_and_prepare(self, dl_manager, **prepare_split_kwargs):
    ...
    for split_generator in self._split_generators(dl_manager):
      ...
      shard_lengths = self._prepare_split(split_generator,
                                          **prepare_split_kwargs)
      split_generator.split_info.shard_lengths.extend(shard_lengths or [])
  ...
  def _as_dataset(self, split=splits_lib.Split.TRAIN, shuffle_files=None):
    if self.version.implements(utils.Experiment.S3):
      dataset = self._tfrecords_reader.read(
          self.name, self._data_dir, split, self.info.splits)
    else:
